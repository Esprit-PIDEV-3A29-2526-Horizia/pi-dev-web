...
class PublicationController extends AbstractController
{
    ...
    public function index(PublicationRepository $repo, PaginatorInterface $paginator, Request $request): Response
    {
        ...
        $favoritesIds = $request->getSession()->get('favorite_publications', []); // AJOUT

        return $this->render('front/publication/index.html.twig', [
            'publications' => $publications,
            'likedIds' => $likedIds,
            'favoritesIds' => $favoritesIds, // AJOUT
        ]);
    }
    ...
    {
        $user = $this->getUser();
        if (!$user || ($user->getUserIdentifier() !== $publication->getAuteur() && !$this->isGranted('ROLE_ADMIN'))) {
